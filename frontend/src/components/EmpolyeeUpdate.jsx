...
const navigate = useNavigate();
    ...
    const initialData = {
        ...
        gender: '',
        course: [],
        image: null // You can provide the URL of an image if available
      };
  ...
  const fetchData = async (id) => {
    try {
      // Make an API call to fetch data based on the id
  ...
  const handleSubmit = async(e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      try {
        ...
        const response = await axios.post(`http://localhost:3000/employeeUpdate/${id}`,formData,{
          headers:{
